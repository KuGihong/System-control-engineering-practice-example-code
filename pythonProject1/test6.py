...
# from the command line where a2 a1 a0 are the coefficients of the char poly or you may edit the string below

#############################################################################################################
# change this to the coefficients of your char poly if you are not using the command line
# varin = '3 (24-4*kp) (48-4*ki-28*kp) (8*kp-28*ki) (8*ki+168*kp) (168*ki)'
# varin = '(3-4*kd) (24-28*kd-4*kp) (48+8*kd-28*kp) (8*kp+168*kd) (168*kp)'
# varin = '1 4*kd 4*kp'
varin = '1 (kp+5.45) (11*kp+11.91) (10*kp+43.65)'
...
# if you want to use PID gainset, use_symbol is True
use_symbol = True  # True로 바꾸면 지정한 값을 대입해서 계산 가능
kp = -0.36
kd = 0.05
ki = 0.005
...
def get_row(initial_column, length):
    ...
    # Check and see if the last element of row is a zero. If not. append a zero
    if row[-1] != 0:
        row.append(0)

    # If this is the second row then the length of this row must be the same as the first
    if length > 0:
...
def print_first_column(rh, coeff):
    ...
    # If the coefficients are all floats, let's try and compute the roots
    try:
...
def get_next_row(rh):
    # This will take a routh table and compute the next row using determinants
    # First we need to grab the last two rows
    last_row = rh[-1]
    second_to_last_row = rh[-2]
    # print('Last Two Rows',last_row,second_to_last_row)

    # Then we loop through the number of columns and create determinants.
    # Remember that the determinant is -(a*d - b*c)/e
    ...
    # The numbers b and d however change with column so this is where we loop
    next_row = []
...
for var_str in coeff_split:
    try:
        var_new = float(var_str)
    except:
        if use_symbol == False:
          print('Using symbolic tool box on var = ', var_str)
          var_new = sp.Symbol(var_str)
        else:
          print('Using symbolic value in pre-defined result on var = ', var_str)
          var_new = eval(var_str, {"kp": kp, "kd": kd, "ki": ki})
...
# To generate a row you start with the first element and loop until you overflow
...
# If the system is first order you're done, you skip to printing the 1st column
if order_system == 1:
    print_first_column(routh_table, coeff)
    sys.exit()

# Otherwise it's time to do some determinants
# We're going to make a method to do this
